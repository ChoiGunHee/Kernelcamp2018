...
int log_read_write(struct super_block *sb,struct inode *inode )
{
	...
	sector_t blocknr = 0, log_sectornr = 0;
	int offset;
	loff_t i_pos;

	if(sbi->log_pos > sbi->log_max_pos)
	{
		trace_printk("log_read_write : log is full it needs to be flushed \n");
		return 0;
	}

	log = kmalloc(sizeof(struct fat_log_entry), GFP_NOFS);
	if(!log){
		trace_printk("log_read_write : unable to alloc log \n");
		return 0; 
	}

	i_pos = fat_i_pos_read(sbi, inode);
	if (!i_pos){
		trace_printk("log_read_write : unable to get i_pos \n");
		return 0;
	}
